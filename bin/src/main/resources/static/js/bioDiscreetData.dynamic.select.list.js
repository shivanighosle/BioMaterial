	...
	$("#file" ).on( "change", function() {
		...
		if(fileLocation != '' ){
			...
			if(jsonValue['data']){
				...
				for( var i=1;i<Object.keys(jsonValue.data).length;i++){
					inptString+="<tr>\n" + 
					"";
...
function setValueInHiddenField() {
...
function setValueInHiddenField1() {
...
function addXVariable() {
	...
	if (i < 11) {
		variableCount = i;
		tableWidth += xValue;
		$('.variableName:last').before('<div class="col-sm-1"><form:label  class="text-info text-infoo" path="xVariableId' + i + '" >X-axis variable(' + i + '):</form:label></div>');
		$('.block:last').before('<div class="col-sm-1"><select class="form-control" id="xVariableId' + i + '" name = "xVariableId' + i + '" value="${bioDiscreetDataForm.xVariableId' + i + '}" onclick = "getAllXVariables(this.id)" onchange = "setValueInHiddenField()"></select></div>');
	} else {
		$.alert({
			title: 'Alert!',
